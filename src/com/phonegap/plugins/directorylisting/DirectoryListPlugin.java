 ...
 */
package com.phonegap.plugins.directorylisting;
...
public class DirectoryListPlugin extends Plugin {
	...
	public static final String ACTION2="addToCalendar";
	...
	public PluginResult execute(String action, JSONArray data, String callbackId) {
		...
		if (ACTION2.equals(action)){
			try {
				...
				Log.d("Test", data1.getString(0));
                    // Calendar.addEvent(data1.getString(0),data1.getString(1),data1.getString(2),data1.getString(3),data1.getString(4),
                    // data1.getString(5));
            ...
            resultStr = this.showWebPage(data.getString(0), data.optJSONObject(1));
            
            if (resultStr.length() > 0) {
                status = PluginResult.Status.ERROR;
                return new PluginResult(status, resultStr);
            } else {
                PluginResult pluginResult = new PluginResult(status, resultStr);
        	...
        	resultStr = this.openExternal(data.getString(0), data.optBoolean(1));
            if (resultStr.length() > 0) {
                status = PluginResult.Status.ERROR;
            }
        }else  {
    ...
    public String openExternal(String url, boolean usePhoneGap) {
        try {
            Intent intent = null;
            if (usePhoneGap) {
                intent = new Intent().setClass((Context) cordova,
    ...
    public String showWebPage(final String url, JSONObject options) {
        ...
        Runnable runnable = new Runnable() {
            public void run() {
                dialog = new Dialog((Context) cordova);
               ...
               /* LinearLayout.LayoutParams backParams = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT,LayoutParams.WRAP_CONTENT);
                ...
                LinearLayout main = new LinearLayout((Context) cordova);
                main.setOrientation(LinearLayout.VERTICAL);
                
                LinearLayout toolbar = new LinearLayout((Context) cordova);
              ...
              //  back.setLayoutParams(backParams);
                ...
                ImageButton forward = new ImageButton((Context) cordova);
               ...
               // forward.setLayoutParams(forwardParams);
                ...
                edittext = new EditText((Context) cordova);
                ...
                Button close = new Button((Context) cordova);
         ...
         try {
                   ...
                   close.setImageBitmap(loadDrawable("www/childbrowser/done.png"));
                } catch (IOException e) {
                    Log.e(LOG_TAG, e.getMessage(), e);
                }
                close.setLayoutParams(closeParams);
                                */
                webview = new WebView((Context) cordova);
                webview.getSettings().setJavaScriptEnabled(true);
                webview.getSettings().setBuiltInZoomControls(true);
                WebViewClient client = new ChildBrowserClient(
                        (PhonegapActivity) cordova, edittext);
